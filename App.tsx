...
const App: React.FC = () => {
  ...
  const handleCancelShift = async (pair: ShiftPair) => {
    ...
    // FIX: (L:562) Argument of type 'unknown' is not assignable to parameter of type 'string'.
 ...
 const renderNavigation = () => {
  ...
  const renderAppContent = () => {
    ...
    if (currentUser) {
      // User is identified, show the main application.
      return (
        <>
      ...
      // No user identified. If employees have loaded, show the identification page.
      if (employees.length > 0) {
        return (
          <main className="container">
            {alertComponent}
      ...
      // Employees are still loading, or there are no employees.
      return (
        <main className="container">
          {alertComponent}
